...
function indexBy<T, K extends keyof T & string>(
  map: Map<string, T>,
  key: K,
): Map<T[K], T> {
  ...
  for (const value of map.values()) {
    if (result.has(value[key])) {
      throw new Error(
        `Duplicate indexBy key ${key}: ${value[key]} (${value["id"]}/${result.get(value[key])!["id"]})`,
      );
    }
